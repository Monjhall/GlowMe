...
public class checkActiveEffects extends BukkitRunnable {
	...
	public void run() {
		...
		for (Iterator<Player> playerIterator = onlinePlayers.iterator(); playerIterator.hasNext();) {
			...
			if (!(activePlayer.hasPotionEffect(PotionEffectType.getById(24))))
				continue;

			// If they are glowing, check the time on the effect.
			for (Iterator<PotionEffect> effectsIterator = playerActiveEffects.iterator(); effectsIterator.hasNext();) {
